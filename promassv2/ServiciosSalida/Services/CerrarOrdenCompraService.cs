...
{
    public class CerrarOrdenCompraService
    {
        private static ILog log = LogManager.GetLogger(typeof(CerrarOrdenCompraService));
        private GenericDAO genericDAO = null;
        private const string IVA8 = "IVAA8";

        public CerrarOrdenCompraService()
        {
        ...
        public bool isCancelaciones()
        {
            ...
            string query = @"
                select 
	                TOP 1 1
                from OPOR o
                left join bxp_sync_cierre_ordenes bsp 
                    on o.|DocEntry| = bsp.doc_entry
                    and o.|UpdateDate| = bsp.|LAST_UPDATE|
                where
                    o.|UpdateDate| >= ?
                    and o.|DocStatus| = 'C'
            ...
            using (DbConnection conn = genericDAO.GetConnection())
            {
                ...
                pFecha.Value = DateTime.Today.AddDays(-5);
        ...
        {
            string query = @"
                insert into bxp_sync_cierre_ordenes (doc_entry, log_instance, doc_num, estatus, mensaje, last_update)
                select 
	                o.|DocEntry|,
                    0 as LogInstance,
                    o.|DocNum|,
        ...
        public bool isPendientes()
        {
            ...
            string query = "select top 1 1 from bxp_sync_cierre_ordenes where estatus = 0";
        ...
        public List<OrdenCompraVO> getOrdenesCompra()
        {
            string query = @"
                ...
                FROM bxp_sync_cierre_ordenes bso 
        ...
        public void procesarOrdenesCompra(List<OrdenCompraVO> ordenesList)
        {
            ...
            {
                Console.WriteLine($"{index} - Cerrando OC {ordenCompraVO.DocEntry}");
        ...
        {
            string query = @"
                UPDATE bxp_sync_cierre_ordenes 
